        ...
        { cardsList.length > 0 ? cardsList.map(card => (
          <Grid item lg={2} md={4} sm={6} xs={12} key={card.id}>
            { window.location.href.toString().includes('/custom-heroes') ?
              <Card name={card.fullName} image={card.image} id={card.id} handleOpen={handleOpen} /> :
              <Card name={card.name} image={card.hasOwnProperty("images") ? card.images.lg : ""} id={card.id} handleOpen={handleOpen} />
            }
          </Grid>
